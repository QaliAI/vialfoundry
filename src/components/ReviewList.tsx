  ...
  return (
    <div className="border-t border-white/10 pt-10 space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="font-display text-2xl font-bold text-white">Verified Researcher Reviews</h3>
        {reviews.length > 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {reviews.map((r, i) => (
          <div key={i} className="p-5 rounded-2xl bg-slate-900/70 border border-white/10 space-y-3">
            <div className="flex items-center justify-between">
            ...
            {r.title && <h4 className="font-display text-sm font-bold text-white">{r.title}</h4>}
            <p className="text-xs text-slate-400 font-light leading-relaxed">{r.body}</p>
