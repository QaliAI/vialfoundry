...
export default function CheckoutPage() {
  ...
  if (cart.length === 0) {
    return (
      <div className="pt-32 pb-20 max-w-xl mx-auto px-4 text-center space-y-4">
        ...
        <h2 className="font-display text-2xl font-bold text-slate-900">Your Cart is Empty</h2>
        <p className="text-xs text-slate-500 font-mono">Add reference materials or standards to proceed with checkout.</p>
        <button
          onClick={() => router.push('/catalog')}
          className="px-6 py-3 rounded-xl bg-cyan-700 hover:bg-cyan-800 text-white font-medium text-xs font-display shadow-sm"
        >
          Explore Catalog
        </button>
      </div>
  ...
  return (
    <div className="pt-28 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10 bg-[#FAFAF9]">
      ...
      {submitError && (
        <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs font-mono font-medium">
          {submitError}
        </div>
      )}
      ...
      <form onSubmit={handlePlaceOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="storefront-card p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-4">
            <h3 className="font-display text-lg font-bold text-slate-900">Shipping Address</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-mono">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 focus:outline-none focus:bg-white focus:border-cyan-600"
                />
              </div>
              <div className="space-y-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-mono">
              <div className="space-y-1">
                <label className="text-slate-600 font-medium">Institution / Company (Optional)</label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-3 gap-4 text-xs font-mono">
              ...
              <div className="space-y-1">
                <label className="text-slate-600 font-medium">State / Prov *</label>
                <input
                  type="text" required
              ...
              <div className="space-y-1">
                <label className="text-slate-600 font-medium">ZIP / Postal *</label>
                <input
                  type="text" required
          ...
          <div className="storefront-card p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] font-mono font-medium text-emerald-700">Insulated Packaging</span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`text-left p-4 rounded-xl border transition-all flex items-start justify-between ${
                      isSelected
                        ? 'bg-cyan-50 border-cyan-600/60 ring-1 ring-cyan-600/30'
                        : 'bg-slate-50 border-slate-200 hover:border-slate-300'
                    }`}
                  >
                    <div>
                      <div className="font-display text-sm font-bold text-slate-900">{opt.name}</div>
                      <div className="text-[11px] font-mono text-slate-500 mt-0.5">{opt.description}</div>
                    </div>
          ...
          <div className="storefront-card p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-display text-lg font-bold text-slate-900">Payment Method</h3>
              <span className="text-[11px] font-mono text-slate-500">Select preference</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {PAYMENT_METHODS.map((m) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${active ? 'bg-cyan-700 text-white' : 'bg-white text-cyan-800 border border-slate-200'}`}>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] font-mono text-slate-500">{m.tagline}</div>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
            <label className="flex items-start space-x-3 cursor-pointer">
              <input
                ...
                className="mt-1 rounded bg-white border-slate-300 text-cyan-700 focus:ring-0"
              />
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="storefront-card p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-4 sticky top-24">
            <h3 className="font-display text-lg font-bold text-slate-900">Order Summary</h3>

            {/* Itemized List */}
            <div className="space-y-3 max-h-60 overflow-y-auto pr-1 border-b border-slate-100 pb-4">
              {cart.map(({ product, quantity }) => (
                <div key={product.id} className="flex items-center justify-between text-xs font-mono">
                  <div>
                    <span className="text-slate-900 font-bold">{product.name}</span>
                    <span className="text-slate-500 block text-[10px]">Qty: {quantity} | LOT: {product.lotNumber}</span>
                  </div>
                  <span className="text-slate-900 font-bold">${(product.price * quantity).toFixed(2)}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex space-x-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 rounded-xl bg-slate-50 border border-slate-300 text-slate-900 font-mono text-xs focus:outline-none focus:bg-white focus:border-cyan-600"
                />
                <button
                  onClick={handleApplyDiscount}
                  type="button"
                  className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-xs font-mono font-bold text-slate-800 border border-slate-300 transition-colors"
                >
                  Apply
                </button>
              </div>
              {appliedDiscount && (
                <div className="text-[11px] font-mono text-emerald-700 flex items-center space-x-1">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                  <span>Code {appliedDiscount.code} applied (-${(appliedDiscount.discountCents / 100).toFixed(2)})</span>
                </div>
              )}
              {discountError && (
                <div className="text-[11px] font-mono text-rose-600">{discountError}</div>
              )}
            </div>
            ...
            <div className="space-y-2 text-xs font-mono pt-2">
              ...
              <div className="flex justify-between text-slate-600">
                ...
                <span className={shippingInfo.amountCents === 0 ? 'text-emerald-700 font-bold' : 'text-slate-900'}>
              ...
              <div className="flex justify-between text-slate-900 font-bold text-base pt-3 border-t border-slate-100">
                <span>Total Due</span>
                <span className="text-cyan-800">${grandTotalDollars}</span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl bg-cyan-700 hover:bg-cyan-800 text-white font-display font-bold text-sm shadow-sm flex items-center justify-center space-x-2 transition-all"
            >
            ...
            <div className="text-center text-[11px] font-mono text-slate-500 flex items-center justify-center space-x-1.5">
              ...
              <span>Direct Laboratory Procurement · Instant Confirmation</span>
            </div>
          </div>
        </div>
