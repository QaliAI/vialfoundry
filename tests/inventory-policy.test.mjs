...
test("financial refunds default to no restock; explicit restock stays idempotent", () => {
  const decremented = {
    payment_status: "paid",
    inventory_decremented_at: "2026-01-01T00:00:00Z",
    inventory_restocked_at: null,
  };
  assert.equal(shouldRestockInventory(decremented, true).reason, "restock_not_requested");
  assert.equal(shouldRestockInventory(decremented, false, true).reason, "partial_refund");
  assert.equal(shouldRestockInventory({ ...decremented, inventory_restocked_at: "x" }, true, true).reason, "already_restocked");
  assert.equal(shouldRestockInventory({ payment_status: "paid", inventory_decremented_at: null }, true, true).reason, "never_decremented");
});
...
test("payment 10 → 9 stays at 9 after refund unless restock is explicitly requested", () => {
  let onHand = 10;
  ...
  assert.equal(shouldRestockInventory(decremented, true).apply, false);
  assert.equal(onHand, 9);
  assert.equal(shouldRestockInventory(decremented, true, true).apply, true);
  onHand = nextInventoryQuantity(onHand, 1);
  assert.equal(onHand, 10);
  const restocked = { ...decremented, inventory_restocked_at: "now" };
  assert.equal(shouldRestockInventory(restocked, true, true).apply, false);
});
